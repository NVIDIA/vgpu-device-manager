...
`nvidia-vgpu-dm` detects the framework automatically and uses the same configuration file format and CLI semantics for both.

On systems using the vendor-specific VFIO framework:

- SR-IOV virtual functions are enabled automatically (via the `sriov-manage` script shipped with the vGPU Manager driver) if they are not enabled already.
